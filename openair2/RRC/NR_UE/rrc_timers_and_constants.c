...
void nr_rrc_handle_timers(NR_UE_RRC_INST_t *rrc)
{
  ...
  if(t300_expired) {
    LOG_I(NR_RRC, "[UE %ld] Timer T300 expired! No timely response to RRCSetupRequest\n", rrc->ue_id);
  ...
  if(t301_expired) {
    LOG_I(NR_RRC, "[UE %ld] Timer T301 expired! No timely response to RRCReestabilshmentRequest\n", rrc->ue_id);
  ...
  if (t302_expired) {
    LOG_I(NR_RRC, "[UE %ld] Timer T302 expired! Access barring alleviated!\n", rrc->ue_id);
    handle_302_expired_stopped(rrc);
  }

  bool t304_expired = nr_timer_tick(&timers->T304);
  if(t304_expired) {
    LOG_I(NR_RRC, "[UE %ld] Timer T304 expired\n", rrc->ue_id);
    // TODO
  ...
  if(t310_expired) {
    LOG_I(NR_RRC, "[UE %ld] Timer T310 expired\n", rrc->ue_id);
  ...
  if (t311_expired) {
    LOG_I(NR_RRC, "[UE %ld] Timer T311 expired! No suitable cell found in time after initiation of re-establishment\n", rrc->ue_id);
  ...
  if (t430_expired && rrc->nrRrcState == RRC_STATE_CONNECTED_NR && rrc->is_NTN_UE) {
    LOG_I(NR_RRC, "[UE %ld] Timer T430 expired! Indicate UL SYNC LOSS to MAC\n", rrc->ue_id);
